...
const AccommodationCompareItem: React.FC<Props> = ({ accommodation ,image}) => {
  ...
  getCompareImageUrl()
  let url = "../../accommodation/what-is-a" + (accommodation[0].category).toLowerCase()
  return (
    <div className={classes.container}>
      <h3 className={classes.title}>{accommodation[0].category}</h3>
      <div>
        {accommodation.map((item) => (
          <AccommodationCompareSentence key={item.id} description={item.description} />
        ))}
      </div>
      <div className={classes.imageContainer}>
        <img src={imageUrl} className={classes.image} width={300} height={200} alt="" />
      </div>
      <Link href={url}>
