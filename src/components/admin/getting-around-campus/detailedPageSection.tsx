...
export default function DetailedPageSection() {
  ...
  const fetchPageSections = useCallback(async () => {
    ...
    querySnapshot.forEach((doc) => {
      if (doc.id.startsWith("getting-around-campus-section-")) {
  ...
  const handleDeleteSection = async (id: string) => {
    if (!id) throw Error;
    const docRef = doc(getFirestore(), "getting-around-campus", id);
            ...
            {blogs.map((blog, index) => (
              <div key={blog.id} className={classes.blogContainer}>
                <Link
                  ...
                  href={`/admin/pages/getting-around-campus/${blog.slug}`}
                >
