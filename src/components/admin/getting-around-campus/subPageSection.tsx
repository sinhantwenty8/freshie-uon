...
export default function SubPageSection() {
  ...
  const fetchPageSections = useCallback(async () => {
    ...
    querySnapshot.forEach((doc) => {
      if (doc.id.startsWith("getting-around-campus-" + blogTitle)) {
  ...
  const handleDeleteSection = async (id: string) => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `getting-around-campus/getting-around-campus-section-${id}`
      );
  ...
  const handleDeleteBlog = async (blogId: string) => {
    const docRef = doc(getFirestore(), "getting-around-campus-blog", blogId);
            ...
            {blogs.map((blog, index) => (
              <div key={blog.id} className={classes.blogContainer}>
                <Link
                  ...
                  href={`/admin/pages/getting-around-campus/${blogTitle}/${blog.slug}`}
                >
