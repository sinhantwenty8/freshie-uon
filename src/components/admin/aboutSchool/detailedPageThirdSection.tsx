...
import RenderedThirdSection from "./renderedThirdSection";
...
export default function DetailedPageThirdSection() {
  ...
  const fetchPageSections = useCallback(async () => {
    const querySnapshot = await getDocs(collection(getFirestore(), "tips-aboutus-page"));
    const fetchedPageSections: PageSection[] = [];
    querySnapshot.forEach((doc) => {
      if (doc.id.startsWith("mentions")) {
  ...
  const handleDeleteSection = async (id: string) => {
    if (!id) throw Error;
    const docRef = doc(getFirestore(), "tips-aboutus-page", id);
