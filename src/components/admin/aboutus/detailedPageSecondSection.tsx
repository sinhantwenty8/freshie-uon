...
export default function DetailedPageSecondSection() {
  ...
  const fetchPageSections = useCallback(async () => {
    const querySnapshot = await getDocs(collection(getFirestore(), "aboutus-mid-section"));
    const fetchedPageSections: PageSection[] = [];
    querySnapshot.forEach((doc) => {
      if (doc.id.startsWith("serviceitem")) {
  ...
  const handleDeleteSection = async (id: string) => {
    if (!id) throw Error;
    const docRef = doc(getFirestore(), "aboutus-mid-section", id);
