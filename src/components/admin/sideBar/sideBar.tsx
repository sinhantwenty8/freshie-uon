...
import { addDoc, collection, doc, getDoc, getFirestore, setDoc,serverTimestamp, getDocs, updateDoc, deleteDoc } from "firebase/firestore";
...
export default function SideBar({ postPage, postHeader, postHeaderPreview,postPagePreview,setIsPublished, isPublished, postedBy, postedDate }: SideBarProps) {
  ...
  const postPageSection = async (id : string, title:string, description:string, uploadedImage:string) => {
      ...
      try {
        ...
        const imageUrl = uploadedImage ? await getDownloadURL(storageRef) : uploadedImage;
