...
export default function Preview({}: PreviewProps) {
  ...
  const getBlogHeader = useCallback(async () => {
    const querySnapshot = await getDocs(collection(getFirestore(), "accommodation-blog-header-preview"));
    querySnapshot.forEach((doc) => {
      if (doc.id === blogTitle) {
        const blogHeader: BlogHeader = { title: doc.data().title, description: doc.data().description, imageUrl: doc.data().imageUrl };
  ...
  const getBlog = useCallback(async () => {
    const querySnapshot = await getDocs(collection(getFirestore(), "accommodation-blog-preview"));
    querySnapshot.forEach((doc) => {
      if (doc.id === blogTitle) {
  ...
  return (
    <div>
      <Header title={blogHeader.title} description={blogHeader.description} imageUrl={blogHeader.imageUrl} />
