...
export default function DetailPage() {
  ...
  const fetchPage = useCallback(async () => {
    ...
    const docs = await getDocs(collection(getFirestore(),"accommodation-blog-header"));
    docs.forEach((doc)=>{
      if(doc.id==blogTitle){
  ...
  useEffect(() => {
        ...
        try {
          if (docCurSnap.exists()) {
            if(slug !=blogTitle ){
              ...
              await setDoc(docRef, { slug:slug, title: title, text: text, createdAt: timestamp, postedBy: "Admin" ,isPublishedGlobally:isPublishedGlobally}).then(()=>{
            ...
            await setDoc(docRef, { title: title, text: text, createdAt: timestamp, postedBy: "Admin" });
    ...
    const postPagePreview = async () => {
      if (text === '') {
        ...
        const docCurRef = doc(getFirestore(), "accommodation-blog-preview", blogTitle);
        const docSnap = await getDoc(docRef);
        const docCurSnap = await getDoc(docCurRef)
        ...
        try {
          if (docCurSnap.exists()) {
            if(slug !=blogTitle ){
              await deleteDoc(docCurRef).then(()=>setModalMessage("Page successfully deleted."));
              await setDoc(docRef, { title: title, text: text, createdAt: timestamp, postedBy: "Admin" ,isPublishedGlobally:isPublishedGlobally}).then(()=>{
                router.replace(`/admin/pages/accommodation/blogTitle?=${slug}`)
              });
              setModalMessage("Page successfully updated.");
            }else{
    ...
    const postHeader = async () => {
      ...
      const storageRef = ref(storage, `accommodation/accommodation-${slug}-header-banner/`);
      ...
      try {
        ...
        const docSnap = await getDoc(docRef);
        const curDocRef =doc(getFirestore(), "accommodation-blog-header", blogTitle);
        ...
        if (curDocSnap.exists()) {
          ...
          if (slug != blogTitle) {
            ...
            await updateDoc(docRef, { title: bannerTitle, description: bannerDescription, imageUrl });
          }
          setModalMessage("Page successfully updated.");
        } else {
          await setDoc(docRef, { title: bannerTitle, description: bannerDescription, imageUrl ,slug:slug});
    ...
    const postHeaderPreview = async () => {
      const storage = getStorage();
      const storageRef = ref(storage, `accommodation/accommodation-${slug}-header-banner-preview/`);
      ...
      try {
        ...
        const imageUrl = isImageUploaded ? await getDownloadURL(storageRef) : bannerImageUrl;
        const docRef = doc(getFirestore(), "accommodation-blog-header-preview", slug);
        const docSnap = await getDoc(docRef);
        const curDocRef =doc(getFirestore(), "accommodation-blog-header-preview", blogTitle);
        const curDocSnap = await getDoc(curDocRef);
        if (curDocSnap.exists()) {
          // Delete the existing document if the slug is not the same
          if (slug != blogTitle) {
            await deleteDoc(curDocRef).then(()=>setModalMessage("Header successfully deleted."));
            await setDoc(docRef, { title: bannerTitle, description: bannerDescription, imageUrl,slug:slug}).then(()=>{
                router.replace(`/accommodation/${slug}/preview`)
              });
          }else{
            await updateDoc(docRef, { title: bannerTitle, description: bannerDescription, imageUrl }).then(()=>{
                router.replace(`/accommodation/${slug}/preview`)
              });
          }
          setModalMessage("Page successfully updated.");
      ...
      <div className={classes.container}>
        ...
        <div className={classes.flexContainer}>
          <div className={classes.bodyContainer}> 
            ...
            <SimpleMDE  className={classes.simpleMde} value={text} onChange={(e: string) => { settext(e); seterrorstates({ ...errorstates, emptytext: false }) }} />
            <div style={{ color: 'red' }}>{errorstates.emptytext ? 'Please enter text' : ''}</div>
