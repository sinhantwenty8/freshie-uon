...
export default function DetailPage() {
  ...
  const fetchPage = useCallback(async () => {
    const querySnapshot = await getDocs(collection(getFirestore(), "getting-around-sg-blog"));
  ...
  useEffect(() => {
    ...
    const postPage = async () => {
      if (text === '') {
        ...
        const docCurRef = doc(getFirestore(), "getting-around-sg-blog", blogTitle);
          ...
          if (docCurSnap.exists()) {
            if(slug !=blogTitle ){
              ...
              await setDoc(docRef, { slug:slug, title: title, text: text, createdAt: timestamp, postedBy: "Admin" ,isPublishedGlobally:isPublishedGlobally}).then(()=>{
                router.replace(`/admin/pages/getting-around-sg/blogTitle?=${slug}`)
              });
    ...
    const postHeader = async () => {
      ...
      const storageRef = ref(storage, `getting-around/getting-around-sg-blog-${slug}-header-banner/`);
