...
export default function DetailPage() {
  ...
  const fetchPage = useCallback(async () => {
    const querySnapshot = await getDocs(collection(getFirestore(), "getting-around-sub-page-header"));
  ...
  useEffect(() => {
    ...
    const postPage = async () => {
      if (text === '') {
        ...
        const docRef = doc(getFirestore(), "getting-around-sub-page-header", slug);
        const docCurRef = doc(getFirestore(), "getting-around-sub-page-header", blogTitle);
    ...
    const postHeader = async () => {
      ...
      const storageRef = ref(storage, `getting-around-sg/sub-page-${slug}-header-banner/`);
      ...
      try {
        ...
        const docRef = doc(getFirestore(), "getting-around-sub-page-header", slug);
        const docSnap = await getDoc(docRef);
        const curDocRef =doc(getFirestore(), "getting-around-sub-page-header", blogTitle);
        ...
        if (curDocSnap.exists()) {
          ...
          if (slug != blogTitle) {
            ...
            await setDoc(docRef, { title: bannerTitle, description: bannerDescription, imageUrl:imageUrl,slug:slug});
          }else{
            await updateDoc(docRef, { title: bannerTitle, description: bannerDescription, imageUrl:imageUrl});
          }
          setModalMessage("Page successfully updated.");
        } else {
